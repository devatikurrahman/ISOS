...
-(void) showCurrentNotificationPin {
	...
	MyMapAnnotation *annotaion = [[MyMapAnnotation alloc] init];
	[annotaion setAnnotationData:notificationData];
	[mapAnnotations addObject:annotaion];
...
- (void)viewDidLoad {
	...
	viewName.text = @"Trouver Lieu";
	
	self.mapView.delegate = self;
	self.mapView.mapType = MKMapTypeStandard;
	self.mapView.showsUserLocation = YES;
	
	[self showCurrentNotificationPin];
...
- (MKAnnotationView *)mapView:(MKMapView *)theMapView viewForAnnotation:(id <MKAnnotation>)annotation
{
	...
	CLLocationCoordinate2D centerCoord = {annotation.coordinate.latitude, annotation.coordinate.longitude };
	[self setCenterCoordinate:centerCoord zoomLevel:13 animated:YES];
	...
	if (!pinView)
	{
		...
		/*
		UIButton* rightButton = [[UIButton alloc] init];//[UIButton buttonWithType:UIButtonTypeDetailDisclosure];
		[rightButton setTitle:[annotation title] forState:UIControlStateNormal];
		rightButton.frame = CGRectMake(0, 0, 21, 27);
		//[rightButton setImage:[UIImage imageNamed:@"btn_map_popup.png"] forState:UIControlStateNormal];
